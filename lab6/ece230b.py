...
def zadoff_chu_sequence(N,q):
    ...
    Returns np array of a ZCS
    """
    n = np.arange(N)

    return np.exp(-1j * np.pi * q * ((n ** 2) + n) / N)
    
def get_subcarrier_indices(num_subcarriers=2**10, pilot_spacing=4, num_nulls_dc=1, num_nulls_lower=1, num_nulls_upper=1):
    """
    Even number subcarriers please
    """
    # lowest, highest and DC are nulls
    idx_subs_nulls = [0, num_subcarriers / 2, num_subcarriers - 1]
    # every 4th non null is a pilot
    idx_subs_pilots = [i for i in range(pilot_spacing, int(num_subcarriers/2), pilot_spacing)] + [j for j in range(int(num_subcarriers/2) + pilot_spacing, num_subcarriers, pilot_spacing)]
    idx_all = np.arange(num_subcarriers)
    idx_subs_data = np.setdiff1d(idx_all, idx_subs_nulls + idx_subs_pilots)
    return [idx_subs_data, np.array(idx_subs_pilots), np.array(idx_subs_nulls)]
