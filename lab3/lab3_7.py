...
# ---------------------------------------------------------------
sdr = adi.Pluto(token='lRQCO3MYywE') # create Pluto object
...
for rx_data in rx_data_arr_64:
    ...
    rx_pilot = received[80:80 + (len(pilots) * 10): 10]

    # discard the convolution overhead and down sample
    rx_symbols = received[80 + (len(pilots) * 10):80 + (len(pilots) * 10) + 10000:10]
...
ax.set_title('SER vs SNR for 64-QAM and 256-QAM, Raised Cosine Pulse Shape')
ax.set_xticks(x)
ax.legend()
